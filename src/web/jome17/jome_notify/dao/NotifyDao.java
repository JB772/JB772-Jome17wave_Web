...
public interface NotifyDao {
	...
	FriendListBean getFriendListBean (String notificationBody);
	//用 Attender_No 取得 AttenderBean 物件 (單則揪團成功/失敗的Bean)
	AttenderBean getAttenderBean (String notificationBody);
	//用 Group_ID 與 memberId 取得 ScoreList  (評分列表)
	List<ScoreBean> getScoreList (String notificationBody, String memberId);
	
	//用 Group_ID 取得 GroupBean 物件 (單則揪團資料)
	Group getGroupBean (String notificationBody);
