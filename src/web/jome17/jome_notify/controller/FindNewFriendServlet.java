...
public class FindNewFriendServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		switch (action) {
		/*
		 * 收到Json：{memberId, acceptAccount} 以帳號搜尋id，拿到member物件 帳號錯誤，找不到物件 friendList
		 * true 判斷狀態 1,2,3 false 帳號存在(陌人)，建立好友連結 包裝Json：member物件, friendRelation
		 */
		case "searchMember":
...
//					System.out.println("InviteId: " + jsonIn.get("inviteId").getAsString()); //Log
		...
		/*
		 * 按加入朋友 收到Json:{invitedId, acceptId} 以FriendListBean，判斷 insert 或 update insert
		 * 結果代碼 update 結果代碼 包裝Json:{結果代碼, FriendList物件}
		 */
		case "addNewFriend":
//				int resultCode = 0;
			FriendListBean checkList = new Gson().fromJson(jsonIn.get("addNewFriend").getAsString(),
					FriendListBean.class);
...
//System.out.println("uid是"+changeResult.getuId());
		...
		/*
		 * 按“同意”按鈕 收到Json:{ FriendListBean物件 }
		 * 以FrienListBean物件，取出uId，用uId去找出資料並修改其FriendStatus為1 (成功) update成功 結果代碼1
		 * update失敗 結果代碼0 包裝Json:{結果代碼, FriendList物件}
		 */
		case "clickAgree":
//				
//				int resultCode = 0;
...
//				}
		...
		/*
		 * 按“拒絕”按鈕 收到Json:{ FriendListBean物件 }
		 * 以FrienListBean物件，取出uId，用uId去找出資料並修改其FriendStatus為2 (拒絕！) update成功 結果代碼1
		 * update失敗 結果代碼0 包裝Json:{結果代碼, FriendList物件}
		 * 
		 */
		case "clickDecline":
			FriendListBean declineBean = new Gson().fromJson(jsonIn.get("declineBean").getAsString(),
					FriendListBean.class);
...
//				resultCode = new FriendListDaoimpl().update(declineBean);
		...
		case "deleteRelation":
			FriendListBean deleteFriend = new Gson().fromJson(jsonIn.get("deleteFriend").getAsString(),
					FriendListBean.class);
		...
		afterRelation = new FriendListDaoimpl().selectRelation(new FriendListBean(mainId, acceptId));
